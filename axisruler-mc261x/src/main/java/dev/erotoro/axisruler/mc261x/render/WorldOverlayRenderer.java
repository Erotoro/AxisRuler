...
public final class WorldOverlayRenderer {
    ...
    private void renderSelection(LevelRenderContext context) {
        ...
        try {
            ...
            if (hasPinned) {
                renderPinned(matrices, consumers, config, state, currentWorldKey);
            }

            if (hasPreview) {
                renderPreview(matrices, consumers, config, state, pointA, previewTarget);
            }
    ...
    private void renderPinned(
        ...
        for (Pinned entry : state.pinned().listForWorld(currentWorldKey)) {
            ...
            if (result.valid()) {
                renderStaticBox(matrices, consumers, config, result, entry.colorArgb(), 1.0F, state.labelsEnabled());
            }
        }
    }
    ...
    private void renderPreview(
        ...
        if (result.valid()) {
            renderStaticBox(matrices, consumers, config, result, withAlpha(config.boxColorArgb(), 130), 0.5F, state.labelsEnabled());
        }
    }
    ...
    private void renderStaticBox(
        ...
        VoxelShape shape = Shapes.box(0.0D, 0.0D, 0.0D, result.sizeX(), result.sizeY(), result.sizeZ());
        ...
        Vec3 cameraPos = client.gameRenderer.getMainCamera().position();
        Vec3 labelPos = depthBiased(new Vec3(center.x(), center.y(), center.z()), cameraPos);
    ...
    private void renderDimensionCallouts(
        ...
        if (ENABLE_LABEL_SANITY_CHECK) {
            ...
            RenderPrimitives.drawWorldLabel(
                    ...
                    client.gameRenderer.getMainCamera(),
                    "TEST",
                    new Vec3(center.x(), center.y() + 1.1D, center.z()),
                    TEST_LABEL_SCALE,
                    0xFFFFFFFF,
                    config.labelBackgroundModeEnum(),
    ...
    private Vec3 blockCenter(BlockPoint pos) {
