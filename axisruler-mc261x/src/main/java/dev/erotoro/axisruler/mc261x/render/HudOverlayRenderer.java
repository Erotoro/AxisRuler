package dev.erotoro.axisruler.mc261x.render;
...
public final class HudOverlayRenderer {
    ...
    private void renderHud(GuiGraphicsExtractor context, DeltaTracker tickCounter) {
        ...
        int pinnedCount = state.pinned().countForWorld(client.level.dimension().identifier().toString());
        if (pinnedCount > 0) {
    ...
    private HudData withPinnedRow(HudData base, int pinnedCount, Font textRenderer, AxisRulerConfig config) {
