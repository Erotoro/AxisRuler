package dev.erotoro.axisruler.mc1218.render;
...
public final class WorldOverlayRenderer {
    ...
    private void renderSelectionFill(
        ...
        RenderPrimitives.drawFilledBox(matrices, consumers, minX, minY, minZ, maxX, maxY, maxZ, config.boxFillColorArgb());
    }
    ...
    private void renderDimensionCallouts(
        ...
        Vec3d cameraPos = context.camera().getPos();
        Quaternionf cameraOrientation = context.camera().getRotation();
        CalloutLayout layout = calloutLayout(result, cameraPos, config);

        renderCallout(matrices, consumers, textRenderer, cameraPos, cameraOrientation, layout.x(), Integer.toString(result.sizeX()), config.guideColorArgb(config.xGuideColorArgb()), config);
        renderCallout(matrices, consumers, textRenderer, cameraPos, cameraOrientation, layout.y(), Integer.toString(result.sizeY()), config.guideColorArgb(config.yGuideColorArgb()), config);
        renderCallout(matrices, consumers, textRenderer, cameraPos, cameraOrientation, layout.z(), Integer.toString(result.sizeZ()), config.guideColorArgb(config.zGuideColorArgb()), config);
